...
방법 : 모 게임의 스테이터스(체력, 수분, 에너지)화면 텍스트 부분을 인식. 스탯이 낮을경우 인벤토리 이동. 인벤토리내의 아이템과 사용하고자 하는 아이템의 이미지를 비교. 이미지 일치하면 마우스 이동하여 아이템 사용. 스탯점검 뒤 반복작업 수행. 작업 수행시간이 랜덤하기 때문에, 작업 수행 명령이후 일정시간이 지나면 현재 화면상태 인식하여 작업중인지 판단. 작업중일경우 대기. 작업 완료할 경우 스탯확인 후 다시 작업시작.
